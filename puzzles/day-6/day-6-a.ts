...
function minWinningTime(maxTime: number, recordDistance: number) {
  return (maxTime - Math.sqrt(Math.pow(maxTime, 2) - 4 * recordDistance)) / 2 + 0.000001 // we need to be a bit above this;
}

function maxWinningTime(maxTime: number, recordDistance: number) {
  return (maxTime + Math.sqrt(Math.pow(maxTime, 2) - 4 * recordDistance)) / 2 - 0.000001 // we need to be a bit below this;
}
