...
export async function day9a(dataPath?: string) {
  ...
  return _.reduce(samples.map(predictPrevious), (a, b) => a + b, 0);
}
...
function predictPrevious(sample: number[]) {
  ...
  for (let diffDepth = diffs.length - 1; diffDepth >= 0; diffDepth -= 1) {
    ...
    prevDiffInRow = diffLayer[0] - prevDiffInRow;
  }

  return prevDiffInRow;
}
